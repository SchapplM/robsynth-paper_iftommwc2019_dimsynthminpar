...
Table\,\ref{tab:computation} gives an estimate on the computational cost for the dynamics for three different robots: A 4-axis SCARA robot, an 6-dof industrial robot arm manipulator and the 7-axis DLR/Kuka Leight Weight Robot.
The upper part of the table shows the properties of the robots kinematic and dynamics parameters and the lower part summarizes the computational cost of the dynamics using direct calculation or just the regressor multiplication.
...
Optimization is done by\textsc{Maple} and the number of operations contains the sum of additions, multiplications and assignments of temporary variables. % TODO: Sinus?
The triangular form (\ref{equ:regressor_triangular}) of the regressor, which can be recognized from the ratios of zeros in the table, was also exploited in the code optimization.
...
\begin{itemize} 
\item saving the $\mathrm{dim}(\bm{q}) \times \mathrm{dim}(\bm{\beta})$ regressor matrices for the full trajectory leads to additional overhead in the memory,
