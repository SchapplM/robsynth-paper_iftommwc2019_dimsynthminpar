...
With the assumptions from Sec.\,\ref{sec:DimSynth} of an unchangeable task space trajectory and a separate optimization of kinematic and dynamics parameters, the presented properties of the dynamics equations from Sec.\,\ref{sec:DynMdl} can be used to improve the computation speed of the dimensional synthesis.

Since the inverse kinematics can be calculated in the second loop (over the kinematics parameters) and the dynamics equations are evaluated in a third loop (of the drive train optimization), the joint space trajectory $\bm{q}(t)$ can be regarded as given in the inner loop.
Then the property (\ref{equ:invdyn}) of linearity of the dynamics equation can be used to divide it into the calculation of the regressor matrix $\bm{\Phi}$ in the second loop and the multiplication of the regressor with the dynamics parameter vector  $\bm{\delta}$ in the third loop, as depicted in Fig.\,\ref{fig:dimsynth_dyn_plin}.
...
\begin{table}[tb]
    ...
    \begin{tabular}[t]{|c|c|c|c|} 
        ...
        \hline
        number of joint DoF  & 4 & 6 & 7 \\
        degree of kinematic redundancy & 0 & 0 & 1 \\
        number of kinematic parameters & 2 & 6 & 3 \\
        number of base inertial parameters & 10 & 38 & 45 \\
        ratio of zeros in the regressor & 19/40 & 87/228 & 124/315 \\
        \hline
